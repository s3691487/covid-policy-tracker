...
function setcurrentMonthYear(month) {
...
.controller('MainController', ['$scope', function($scope) {
  ...
  eventClick: function(date) {
    console.log(date['year']+"-"+date['_month']+"-"+date['day']);
    setcurrentDate(date)
  },
  dateClick: function(date) {
    var mm,dd;
    
    if(date['_month']<10) mm='0'+date['_month'];
    else
      mm = date['_month']
    if(date['day']<10) dd='0'+date['day'];
    else
      dd = date['day'] 
    console.log(date['year']+"-"+mm+"-"+dd);
    setcurrentDate(date['year']+"-"+mm+"-"+dd)
    var selectBox = document.getElementById("dropdown");
    var selectedValue = selectBox.options[selectBox.selectedIndex].value;
    getDefaultPolicy('https://q2ua1gzvsj.execute-api.ap-southeast-2.amazonaws.com/beta_0_0/getdailypolicy?country_name='+selectedValue+'&date='+getCurrentDate())
    getCovidStats('https://cxfqobcqe8.execute-api.ap-southeast-2.amazonaws.com/dep_1/getCovidCases?country_name='+selectedValue+'&date='+getCurrentDate())  
    
  },
  changeMonth: function(month, year) {
    var mm;
    var month1 = month['name'].toUpperCase();
    var months = ["JANUARY", "FEBRUARY", "MARCH", "APRIL", "MAY", "JUNE", "JULY", "AUGUST", "SEPTEMBER", "OCTOBER", "NOVEMBER", "DECEMBER"];
    month1 = months.indexOf(month1)+1;
    if(month1<10) mm='0'+month1
    else
      mm = month1
    ; 

    setcurrentMonthYear(year+"-"+mm)
    console.log(month1+" "+year+"-"+mm)
    var selectBox = document.getElementById("dropdown");
    var selectedValue = selectBox.options[selectBox.selectedIndex].value;
    getDefaultPolicy('https://q2ua1gzvsj.execute-api.ap-southeast-2.amazonaws.com/beta_0_0/getmonthlypolicy?country_name='+selectedValue+'&month='+getCurrentMonthYear())
    getCovidStats('https://cxfqobcqe8.execute-api.ap-southeast-2.amazonaws.com/dep_1/getCovidCases?country_name='+selectedValue+'&date='+getCurrentMonthYear()+"-01")  
  },
};
